<?php
/**
 * Löschen des ausgewählten Beitrages
 * @author Max Stötzner
 */
...
$sql = $db->prepare("SELECT COUNT(p_name) FROM pictures WHERE B_ID = ?");
$sql->execute(array($P_B_ID)) or die("Fehler Abfrage Datenbank<br>");
while($row = $sql->fetch()){
  $counter = $row['COUNT(p_name)'];
}
//Löschen des Bildes
 for($i = 0; $i <= $counter; $i++){
...
$sql->execute(array($P_B_ID)) or die("Fehler Abfrage Datenbank<br>");
  while($row = $sql->fetch()){
    ...
    $sql = $db->prepare("DELETE FROM pictures WHERE p_name = ?");
    $sql->execute(array($p_name)) or die("<script> alert('Bild Löschen Fehlgeschlagen') </script>");
 ...
 */
update_B_ID($B_ID, $sql_erg, $db);


/**
 * Aktualisert alle nachfolgenden Beiträge bezüglich ihrer Beitrags ID
 * @param B_ID Beitrags ID die aktualisiert werden soll
 * @param sql_erg Höchste Beitrags ID
 * @param db Datenbank
 */
function update_B_ID($B_ID, $sql_erg, $db){
for($i = $B_ID + 1; $i<=$sql_erg; $i++){
